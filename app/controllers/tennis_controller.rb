class TennisController < ApplicationController
  ...
  # GET /tennis.json
  def index
    @tennis = Tenni.order("points DESC").all
  ...
  def show
    ...
    @mesMatchs = Matchsindiv.where(:email1 => @tenni.email)
    @mesAmis = Ami.where(:email1 => @tenni.email, :sport => "Tennis").select("email2").all
    @moi = User.where(:email => @tenni.email).first
    @joueursAutourDeMoi = User.where(:ville => @moi.ville).all
    respond_to do |format|
